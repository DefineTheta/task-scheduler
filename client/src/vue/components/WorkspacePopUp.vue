...
export default {
  data() {
    return {
      tab: 'new',
      tabStyle:
        'p-3 flex flex-row items-center font-semibold border-r-2 border-grey-dark focus:outline-none',
      workspaceName: '',
      searchWorkspaceName: '',
      errorMessage: '',
      errorPresent: false,
  ...
  created() {
    Event.$on('workspace-creation-failed', (data) => {
    ...
    Event.$on('workspace-creation-success', () => {
      ...
      this.successMessage = 'Workspace was successfully created!';
    });

    Event.$on('workspace-search-failed', (data) => {
      this.successPresent = false;
      this.errorPresent = true;
      this.errorMessage = data.error;
    });

    Event.$on('workspace-search-success', (data) => {
      this.errorPresent = false;
      this.searchedWorkspaces = data.workspaces;
    });

    Event.$on('workspace-join-failed', (data) => {
      this.successPresent = false;
      this.errorPresent = true;
      this.errorMessage = data.error;
    });
    ...
    Event.$on('workspace-join-success', () => {
      ...
      this.errorPresent = false;
      this.successPresent = true;
      this.successMessage = 'You joined the workspace successfully!';
    });
  },
  methods: {
    changeTab(tab) {
    ...
    searchWorkspace() {
      API.post(
        '/workspace/search',
        { workspace_name: this.searchWorkspaceName },
    ...
    <div class="w-116 min-h-80 flex flex-col bg-white shadow-4dp fixed z-20">
      <div class="w-full h-12 bg-side-grey flex flex-row justify-between">
        <div class="flex flex-row">
          <button
            title="New Workspace"
            :class="[tabStyle, tab === 'new' && 'font-bold bg-white']"
            @click="changeTab('new')"
          >
            <svg
              width="21"
          ...
          <button
            ...
            :class="[tabStyle, tab === 'search' && 'font-bold bg-white']"
      ...
      <form v-if="tab === 'new'" class="m-3 flex flex-col" onsubmit="return false;">
        <div class="text-lg font-bold">Create A New Workspace</div>
        <li v-if="errorPresent" class="font-semibold text-red-600 text-sm">
          {{ errorMessage }}
        </li>
        <span v-if="successPresent" class="font-semibold text-green-600 text-sm">
          {{ successMessage }}
        </span>
      ...
      <form v-if="tab === 'search'" class="m-3 flex flex-col" onsubmit="return false;">
        <div class="text-lg font-bold">Search For Workspace</div>
        <li v-if="errorPresent" class="font-semibold text-red-600 text-sm">
          {{ errorMessage }}
        </li>
        <span v-if="successPresent" class="font-semibold text-green-600 text-sm">
        ...
        <div class="flex flex-col items-center">
          <input
            v-model="searchWorkspaceName"
            type="text"
            name="username"
            class="mt-2 w-full h-10 px-1 bg-textbox-grey text-sm font-semibold focus:outline-none"
            placeholder="Name"
          />
          <div v-if="searchedWorkspaces.length > 0" class="w-full mt-1">
            <div
            ...
            >
              ...
              <button
                ...
                @click="joinWorkspace(workspace.workspace_id)"
              >
                <svg
                  width="16"
                  height="16"
