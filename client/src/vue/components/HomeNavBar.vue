...
export default {
  ...
  methods: {
    submitForm() {
      API.post(`/api/v1/${this.accountType}/login`, {}, 'login-complete');
    },
  },
};
</script>
...
<template>
  ...
  >
    ...
    <form class="mr-4 flex flex-row items-center" onsubmit="return false;">
      <input
        type="text"
        name="username"
        class="w-40 h-6 px-1 text-sm font-semibold focus:outline-none"
        placeholder="Username"
      />
      <input
        type="password"
        name="pass"
        class="w-40 h-6 ml-4 px-1 text-sm font-semibold focus:outline-none"
